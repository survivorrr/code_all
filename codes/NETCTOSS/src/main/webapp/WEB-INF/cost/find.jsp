...
<!-- 我试了jsp放在WEB-INF下他不能用网址直接访问例如http://localhost:8080/NETCTOSS/find.jsp -->
<!-- 但如果直接放在webapp下就可以用网址http://localhost:8080/NETCTOSS/find.jsp直接访问 -->
...
<body>
	...
	<div id="navi">
		...
		<!-- 	taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c" 外面百分号加@-->
	...
	<div id="main">
		<form action="" method="">
			...
			<div class="search_add">
				...
				<!-- 
                    	当前：NETCTOSS/findCost.do   这个路径不能写jsp，因为是从浏览器输入/findCost.do,
                    	           然后servlet转发到的jsp，浏览器不知道转发这回事。
			...
			<div id="data">
				<table id="datalist">
					...
					<c:forEach items="${cost_list }" var="c">
						<tr>
							<td>${c.costId }</td>
							...
							<td>${c.baseCost }</td>
							<td>${c.unitCost }</td>
							<!--       <fmt:formatDate value="${c.creatime }" /> -->
							<td><fmt:formatDate value="${c.creatime}"
									pattern="yyyy-MM-dd HH:mm:ss" /></td>
							<!--      <td>${c.creatime }</td>   -->
							<td>${c.startime }</td>
							<td><c:if test="${c.status==0 }">开通</c:if> <c:if
									test="${c.status==1 }">暂停</c:if></td>
							<td><input type="button" value="启用" class="btn_start"
								onclick="startFee();" /> <input type="button" value="修改"
								class="btn_modify"
								onclick="location.href='toUpdateCost.do?id=${c.costId }';" /> <input
